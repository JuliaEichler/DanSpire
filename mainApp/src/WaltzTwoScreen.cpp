...
void WaltzTwoScreen::setup()
{
    ...
    cX1 = config.mainWindowWidth /2 - boxWidth;
    cY1 = config.mainWindowHeight/2 + boxHeight;
}


//--------------------------------------------------------------
void WaltzTwoScreen::update(float dt)
{
    ofSoundUpdate();
}

//--------------------------------------------------------------
void WaltzTwoScreen::draw()
{
    ...
    if(player.getPosition() < 1.0)
    {
                 ...
                 && currentPlayerPos <= stopPlayerPos_R2)
        {
               ...
               && currentPlayerPos < startPlayerPos_R2+intervall+intervall)
            {
                ...
                ofTranslate(cX1+1.6*boxWidth, cY1);
                ofRotateZ(config.rotateZ);
                Foot(mFootA.x, mFootA.y-2*config.distanceMoveTwoWaltz, getStepSide(role, mStepA), 'f', config.iconSize); // left
            ...
            if(currentPlayerPos >= startPlayerPos_R2+intervall+intervall)
            {
                ...
                ofRotateZ(config.rotateZ);
                mGender = 'f';
                Foot(mFootA.x, mFootA.y-2*config.distanceMoveTwoWaltz, getStepSide(role, mStepA), mGender, config.iconSize);
                Foot(mFootB.x, mFootB.y-2*config.distanceMoveTwoWaltz, getStepSide(role, mStepB), mGender, config.iconSize);
                Hand(mFootA.x+180, mFootA.y-2*config.distanceMoveTwoWaltz-55, 'r', mGender, config.iconSize);
                Hand(mFootB.x-130, mFootB.y-2*config.distanceMoveTwoWaltz-80, 'l', mGender, config.iconSize);
                ofPopMatrix();
            }
        }
...
void WaltzTwoScreen::sceneWillAppear( ofxScene * fromScreen )
{
    ofSoundStopAll();
...
//--------------------------------------------------------------
//scene notifications
void WaltzTwoScreen::sceneWillDisappear( ofxScene * toScreen )
{
    
}

//--------------------------------------------------------------
void WaltzTwoScreen::startSound()
{
    player.play();
...
//--------------------------------------------------------------
int WaltzTwoScreen::getPlayerPositionMS()
{
    return player.getPositionMS();
}

//--------------------------------------------------------------
float WaltzTwoScreen::getPlayerPosition()
{
    return player.getPosition();
}

void WaltzTwoScreen::standardPosition(vector< vector<int> > choreo, int position, int x1, int y1, int x2, int y2)
{
    ofPushMatrix();
    ofTranslate(x1, y1);
    ofRotateZ(config.rotateZ);
...
//--------------------------------------------------------------
void WaltzTwoScreen::getChoreoPosition(int step, char gender, bool displayPrevious, vector< vector<int> > choreo, bool bdisplayIcon)
{
    // CURRENT POSITION ----------------------------------------
    char side;
    if(choreo[step][2] == 0)
...
//--------------------------------------------------------------
void WaltzTwoScreen::updateChoreoPosition(vector< vector<int> > choreo, int step, bool bdisplayPrevious, bool bdisplayIcon, bool bcloser, bool bbox)
{
    // CURRENT POSITION ------------------------
    char side;
    if(choreo[step][2] == 0)
    ...
    // get distance
    int d=0;
    if(bcloser) d=config.distanceMoveTwoWaltz;
...
//--------------------------------------------------------------
void WaltzTwoScreen::tweenPositionMaster(vector<ofVec2f> fP, ofVec2f posUpdate, ofVec2f positionPrev, ofVec2f positionCurr, vector< vector<int> > choreo, int step, char side, char gender, bool bdisplayIcon, bool bdisplayMaster, bool bdisplayPrevious, bool bmoveForward, bool bcloser)
{
    if(fP.size() == 0)
    {
        fP.resize(1);
...
//--------------------------------------------------------------
void WaltzTwoScreen::tweenPositionSlave(vector<ofVec2f> fP, ofVec2f posUpdate, ofVec2f positionCurr, vector< vector<int> > choreo, int step, char side, char gender, bool bdisplayIcon, bool bdisplayPrevious, bool bmoveForward, bool bcloser)
{
    if (posUpdate != positionCurr)
    {
        int last = fP.size()-1;
        ...
        /*
         ...
         Hand(fP.at(last).x+80, fP.at(last).y-2*config.distanceMoveTwoWaltz-55, 'r', gender, config.iconSize);
         }
         ...
         Hand(previousHandPos.x, previousHandPos.y-2*config.distanceMoveTwoWaltz-80, 'l', gender, config.iconSize);
         }
         }
         else
         {
         ...
         Hand(fP.at(last).x, fP.at(last).y-2*config.distanceMoveTwoWaltz-80, 'l', gender, config.iconSize);
         }
         ...
         Hand(previousHandPos.x+80, previousHandPos.y-2*config.distanceMoveTwoWaltz-55, 'r', gender, config.iconSize);
         }
         }
...
//--------------------------------------------------------------
ofVec2f WaltzTwoScreen::moveForwardsMaster(vector< vector<int> > choreo, int mStep, char gender)
{
    ofVec2f origin = getStepPosition(choreo, mStep);
    char side = getStepSide(choreo, mStep);
    ofVec2f target;
    target.x = origin.x;
    target.y = origin.y+config.distanceMoveTwoWaltz;
...
//--------------------------------------------------------------
void WaltzTwoScreen::moveForwardsSlave(ofVec2f target, vector< vector<int> > choreo, int mStep, char gender)
{
    ofVec2f origin = getStepPosition(choreo, mStep);
    char side = getStepSide(choreo, mStep);
    ...
    else
    {
        if(gender == 'f')
        {
            Foot(footPositions.at(footPositions.size()-1).x, footPositions.at(footPositions.size()-1).y - 2*config.distanceMoveTwoWaltz, side, gender, config.iconSize);
        }
        else{
...
//--------------------------------------------------------------
ofVec2f WaltzTwoScreen::getStepPosition(vector< vector<int> > choreo, int step)
{
    ofVec2f position;
    if(choreo[step][2] == 0)
    {
...
//--------------------------------------------------------------
char WaltzTwoScreen::getStepSide(vector< vector<int> > choreo, int step)
{
    char side;
    if(choreo[step][2] == 0)
    {
...
//--------------------------------------------------------------
void WaltzTwoScreen::createIcon(vector<ofVec2f> fP, vector< vector<int> > choreo, int step, char gender)
{
    int last = fP.size()-1;
...
//--------------------------------------------------------------
void WaltzTwoScreen::createIcon(ofVec2f position, vector< vector<int> > choreo, int step, char gender)
{
    if(getIcon(choreo[step][3], gender) != "")
    {
        icon.loadImage(getIcon(choreo[step][3], gender));
...
//--------------------------------------------------------------
int WaltzTwoScreen::roundUp(float number)
{
...
//--------------------------------------------------------------
string WaltzTwoScreen::getIcon(int iconId, char gender)
{
    string role;
    if (gender == 'f') {
        role = "follower";
