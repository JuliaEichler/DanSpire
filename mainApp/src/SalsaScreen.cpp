...
//--------------------------------------------------------------
void SalsaScreen::update(float dt)
{
    ofSoundUpdate();
}
...
void SalsaScreen::sceneWillAppear( ofxScene * fromScreen )
{
    ofSoundStopAll();
...
//--------------------------------------------------------------
//scene notifications
void SalsaScreen::sceneWillDisappear( ofxScene * toScreen )
{
    
}

//--------------------------------------------------------------
void SalsaScreen::startSound()
{
    player.play();
...
//--------------------------------------------------------------
int SalsaScreen::getPlayerPositionMS()
{
    return player.getPositionMS();
}

//--------------------------------------------------------------
float SalsaScreen::getPlayerPosition()
{
    return player.getPosition();
}

void SalsaScreen::standardPosition(vector< vector<int> > choreo, int position, int x1, int y1, int x2, int y2)
{
    ofPushMatrix();
    ofTranslate(x1, y1);
    ofRotateZ(config.rotateZ);
...
//--------------------------------------------------------------
void SalsaScreen::updateChoreoPosition(vector< vector<int> > choreo, int step, bool bdisplayPrevious, bool bdisplayIcon, bool bcloser, bool bbox)
{    
    // CURRENT POSITION ------------------------
    char side;
    if(choreo[step][3] == 5)
    ...
    // get distance
    int d=0;
    if(bcloser) d=config.distanceMoveSalsa;
...
//--------------------------------------------------------------
void SalsaScreen::tweenPositionMaster(vector<ofVec2f> fP, ofVec2f posUpdate, ofVec2f positionPrev, ofVec2f positionCurr, vector< vector<int> > choreo, int step, char side, char gender, bool bdisplayIcon, bool bdisplayMaster, bool bdisplayPrevious, bool bmoveForward, bool bcloser)
{
    if(fP.size() == 0)
    {
        fP.resize(1);
...
//--------------------------------------------------------------
void SalsaScreen::tweenPositionSlave(vector<ofVec2f> fP, ofVec2f posUpdate, ofVec2f positionCurr, vector< vector<int> > choreo, int step, char side, char gender, bool bdisplayIcon, bool bdisplayPrevious, bool bmoveForward, bool bcloser)
{
    // if lift
...
//--------------------------------------------------------------
ofVec2f SalsaScreen::moveForwardsMaster(vector< vector<int> > choreo, int mStep, char gender)
{
    ofVec2f origin = getStepPosition(choreo, mStep);
    char side = getStepSide(choreo, mStep);
    ofVec2f target;
    target.x = origin.x;
    target.y = origin.y+config.distanceMoveSalsa;
...
//--------------------------------------------------------------
void SalsaScreen::moveForwardsSlave(ofVec2f target, vector< vector<int> > choreo, int mStep, char gender)
{
    ofVec2f origin = getStepPosition(choreo, mStep);
    char side = getStepSide(choreo, mStep);
    ...
    else
    {
        if(gender == 'f')
        {
            ...
            Foot(footPositions.at(footPositions.size()-1).x, footPositions.at(footPositions.size()-1).y - 2*config.distanceMoveSalsa, side, gender, config.iconSize);
            ofPopMatrix();
        }
        else{
...
//--------------------------------------------------------------
ofVec2f SalsaScreen::getStepPosition(vector< vector<int> > choreo, int step)
{
    ofVec2f position;
    if(choreo[step][2] == 0)
    {
...
//--------------------------------------------------------------
char SalsaScreen::getStepSide(vector< vector<int> > choreo, int step)
{
    char side;
    if(choreo[step][2] == 0)
    {
...
//--------------------------------------------------------------
void SalsaScreen::createIcon(vector<ofVec2f> fP, vector< vector<int> > choreo, int step, char gender)
{
    int last = fP.size()-1;
...
//--------------------------------------------------------------
void SalsaScreen::createIcon(ofVec2f position, vector< vector<int> > choreo, int step, char gender)
{
    if(getIcon(choreo[step][3], gender) != "")
    {
        icon.loadImage(getIcon(choreo[step][3], gender));
...
//--------------------------------------------------------------
int SalsaScreen::roundUp(float number)
{
...
//--------------------------------------------------------------
string SalsaScreen::getIcon(int iconId, char gender)
{
    string role;
    if (gender == 'f') {
        role = "follower";
