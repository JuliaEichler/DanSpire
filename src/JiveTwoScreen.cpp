...
void JiveTwoScreen::setup()
{
    ...
    cX1 = config.mainWindowWidth*0.5 - boxHeight*1.2;
    cX2 = config.mainWindowWidth*0.5 + boxHeight*1.2;
    cY1 = config.mainWindowHeight/2   + boxWidth/2;
...
//--------------------------------------------------------------
void JiveTwoScreen::update(float dt)
{
    ofSoundUpdate();
}

//--------------------------------------------------------------
void JiveTwoScreen::draw()
{
    ...
    if(player.getPosition() < 1.0)
    {
        ...
        {
            standardPosition(role, 7, cX1, cY1, cX2, cY1);
                 ...
                 && currentPlayerPos <= stopPlayerPos_R2)
        {
               ...
               && currentPlayerPos < startPlayerPos_R2+intervall+intervall)
            {
                ...
                ofTranslate(cX2, cY1);
                ofRotateZ(config.rotateZ);
                Foot(mFootA.x, mFootA.y-2*config.distanceMoveTwoJive, getStepSide(role, mStepA), 'f', config.iconSize); // left
                ofTranslate(-boxWidth*2/7, 0);
            ...
            if(currentPlayerPos >= startPlayerPos_R2+intervall+intervall)
            {
                ...
                ofRotateZ(config.rotateZ);
                mGender = 'f';
                Foot(mFootA.x, mFootA.y-2*config.distanceMoveTwoJive, getStepSide(role, mStepA), mGender, config.iconSize);
                Hand(mFootA.x+180, mFootA.y-2*config.distanceMoveTwoJive-55, 'r', mGender, config.iconSize);
                ofTranslate(-boxWidth*2/7, 0);
                Foot(mFootB.x, mFootB.y-2*config.distanceMoveTwoJive, getStepSide(role, mStepB), mGender, config.iconSize);
                Hand(mFootB.x-130, mFootB.y-2*config.distanceMoveTwoJive-80, 'l', mGender, config.iconSize);
                ofPopMatrix();
            }
        }
...
void JiveTwoScreen::sceneWillAppear( ofxScene * fromScreen )
{
    ofSoundStopAll();
...
//--------------------------------------------------------------
//scene notifications
void JiveTwoScreen::sceneWillDisappear( ofxScene * toScreen )
{
    
}

//--------------------------------------------------------------
void JiveTwoScreen::startSound()
{
    player.play();
...
//--------------------------------------------------------------
int JiveTwoScreen::getPlayerPositionMS()
{
    return player.getPositionMS();
}

//--------------------------------------------------------------
float JiveTwoScreen::getPlayerPosition()
{
    return player.getPosition();
}

void JiveTwoScreen::standardPosition(vector< vector<int> > choreo, int position, int x1, int y1, int x2, int y2)
{
    ofPushMatrix();
    ofTranslate(cX1, cY1);
    ofRotateZ(config.rotateZ);
...
//--------------------------------------------------------------
void JiveTwoScreen::getChoreoPosition(int step, char gender, bool displayPrevious, vector< vector<int> > choreo, bool bdisplayIcon, bool bisStart)
{
    // CURRENT POSITION ----------------------------------------
    char side;
    if(choreo[step][2] == 0)
...
//--------------------------------------------------------------
void JiveTwoScreen::updateChoreoPosition(vector< vector<int> > choreo, int step, bool bdisplayPrevious, bool bdisplayIcon, bool bcloser, bool bbox)
{
    // CURRENT POSITION ------------------------
    char side;
    if(choreo[step][3] == 5)
    ...
    // get distance
    int d=0;
    if(bcloser) d=config.distanceMoveTwoJive;
...
//--------------------------------------------------------------
void JiveTwoScreen::tweenPositionMaster(vector<ofVec2f> fP, ofVec2f posUpdate, ofVec2f positionPrev, ofVec2f positionCurr, vector< vector<int> > choreo, int step, char side, char gender, bool bdisplayIcon, bool bdisplayMaster, bool bdisplayPrevious, bool bmoveForward, bool bcloser)
{
    if(fP.size() == 0)
    {
        fP.resize(1);
...
//--------------------------------------------------------------
void JiveTwoScreen::tweenPositionSlave(vector<ofVec2f> fP, ofVec2f posUpdate, ofVec2f positionCurr, vector< vector<int> > choreo, int step, char side, char gender, bool bdisplayIcon, bool bdisplayPrevious, bool bmoveForward, bool bcloser)
{
    // if lift
...
//--------------------------------------------------------------
ofVec2f JiveTwoScreen::moveForwardsMaster(vector< vector<int> > choreo, int mStep, char gender)
{
    ofVec2f origin = getStepPosition(choreo, mStep);
    char side = getStepSide(choreo, mStep);
    ofVec2f target;
    target.x = origin.x;
    target.y = origin.y+config.distanceMoveTwoJive;
...
//--------------------------------------------------------------
void JiveTwoScreen::moveForwardsSlave(ofVec2f target, vector< vector<int> > choreo, int mStep, char gender)
{
    ofVec2f origin = getStepPosition(choreo, mStep);
    char side = getStepSide(choreo, mStep);
    ...
    else
    {
        if(gender == 'f')
        {
            ...
            Foot(footPositions.at(footPositions.size()-1).x, footPositions.at(footPositions.size()-1).y - 2*config.distanceMoveTwoJive, side, gender, config.iconSize);
            ofPopMatrix();
        }
        else{
...
//--------------------------------------------------------------
ofVec2f JiveTwoScreen::getStepPosition(vector< vector<int> > choreo, int step)
{
    ofVec2f position;
    if(choreo[step][2] == 0)
    {
...
//--------------------------------------------------------------
char JiveTwoScreen::getStepSide(vector< vector<int> > choreo, int step)
{
    char side;
    if(choreo[step][2] == 0)
    {
...
//--------------------------------------------------------------
void JiveTwoScreen::createIcon(vector<ofVec2f> fP, vector< vector<int> > choreo, int step, char gender)
{
    int last = fP.size()-1;
...
//--------------------------------------------------------------
void JiveTwoScreen::createIcon(ofVec2f position, vector< vector<int> > choreo, int step, char gender)
{
    if(getIcon(choreo[step][3], gender) != "")
    {
        icon.loadImage(getIcon(choreo[step][3], gender));
...
//--------------------------------------------------------------
int JiveTwoScreen::roundUp(float number)
{
...
//--------------------------------------------------------------
string JiveTwoScreen::getIcon(int iconId, char gender)
{
    string role;
    if (gender == 'f') {
        role = "follower";
